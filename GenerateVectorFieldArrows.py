"""
Arrow-based vector field: draw flow directions; smooth via RBF interpolation.

Run: ``python GenerateVectorFieldArrows.py --image map.png --out field.npy``

Black (or near-black) pixels are obstacles. Click-drag **arrows** on traversable
pixels; ``vx`` and ``vy`` are interpolated with ``scipy`` (RBF / griddata fallback),
optional Gaussian blur, then optional unit normalization.
